...
export default function EventsTable() {
  const { events, isLoading, error, pagination, setPagination } = useEvents();
    ...
    return (
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 backdrop-blur p-12">
        <div className="flex flex-col items-center justify-center gap-2">
          <div className="text-sm text-neutral-400">No events found</div>
          <div className="text-xs text-neutral-500">
            Try adjusting your filters or send some events
          </div>
        </div>
      </div>
    );
          ...
          <table className="w-full">
            <thead className="border-b border-neutral-800/80 bg-neutral-950/60">
              <tr className="text-left">
                <th className="px-6 py-4 text-xs font-bold text-neutral-300 uppercase tracking-wider w-12">
                  {/* Expand column */}
                </th>
                <th className="px-6 py-4 text-xs font-bold text-neutral-300 uppercase tracking-wider">
                  <div className="flex items-center gap-2">Event Name</div>
                </th>
                <th className="px-6 py-4 text-xs font-bold text-neutral-300 uppercase tracking-wider">
                  End User
                </th>
        ...
        <div className="text-sm font-medium text-neutral-300">
          ...
          {pagination.total && (
            <>
              ...
              <span className="text-neutral-500 ml-1">total events</span>
            </>
          )}
        </div>
