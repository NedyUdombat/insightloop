...
import EndUserRow from "./EndUserRow";
import { useEndUsers } from "./EndUsersContext";

export default function EndUsersTable() {
  const { endUsers, isPending, error, pagination, setPagination } =
    useEndUsers();
    ...
    return (
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 backdrop-blur p-12">
        <div className="flex items-center justify-center">
          <div className="text-sm text-neutral-400">Loading end users...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 backdrop-blur p-12">
        <div className="flex items-center justify-center">
          <div className="text-sm text-red-400">
            Error loading end users: {error.message}
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 backdrop-blur p-12">
        <div className="flex flex-col items-center justify-center gap-2">
          <div className="text-sm text-neutral-400">No end users found</div>
          <div className="text-xs text-neutral-500">
            Try adjusting your filters or wait for users to be identified
          </div>
        </div>
      </div>
    );
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-neutral-800/60">
              {endUsers.map((endUser) => (
                <EndUserRow key={endUser.id} endUser={endUser} />
              ))}
            </tbody>
          </table>
        </div>
        ...
        <div className="text-sm font-medium text-neutral-300">
          ...
          {pagination.total && (
            <>
              ...
              <span className="text-neutral-500 ml-1">total users</span>
            </>
          )}
        </div>
